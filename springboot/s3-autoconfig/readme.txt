...
可以通过在配置中添加debug=true来查看哪些自动配置类生效了


springboot启动的时候加载了主配置类，开启了自动配置功能
...
1、自动配置原理
SpringBoot启动的时候加载主配置类，@SpringBootApplication里面包括了@EnableAutoConfiguration，开启了自动配置功能

    2）@EnableAutoConfiguration 作用
        利用EnableAutoConfigurationImportSelector给容器中导入一些组件
        可以查看其中selectImports()方法的内容；
        在getAutoConfigurationEntry方法中获取自动配置条目
        其中由
        List<String> configurations = this.getCandidateConfigurations(annotationMetadata, attributes);
        获取候选的配置
        然后由List<String> configurations =
        SpringFactoriesLoader.loadFactoryNames
        (this.getSpringFactoriesLoaderFactoryClass(), this.getBeanClassLoader());

        loadFactoryNames会调用loadSpringFactories进行扫描所有jar包类路径下的  META-INF/spring.factories
